  ...
  return (
    <header className="site-header">
      <nav className="container navbar" aria-label="Primary navigation">
        <Link className="wordmark" href="/" aria-label="Suhail, home">S<span>.</span></Link>
        <div className="nav-links">{links.map((link) => <Link key={link} href={`/#${link.toLowerCase()}`}>{link}</Link>)}</div>
        <div className="nav-actions"><ThemeToggle /><Link className="button button-small" href={siteConfig.links.resume}>Resume ↗</Link><button className="menu-button" aria-label="Toggle menu" aria-expanded={open} onClick={() => setOpen(!open)}>{open ? "Close" : "Menu"}</button></div>
      </nav>
